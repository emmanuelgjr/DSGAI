...
// Lazy-loaded on download to reduce initial bundle size
async function getDocxModules() {
  const [docxMod, fsMod] = await Promise.all([
    import('docx'),
    import('file-saver'),
  ])
  return { ...docxMod, saveAs: fsMod.saveAs }
}
...
export default function PolicyGenerator() {
  ...
  const handleDownload = async (policyId, title) => {
    ...
    const { Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType, saveAs } = await getDocxModules()

    function textToDocxParagraphs(txt) {
      ...
      for (const line of lines) {
        ...
        if (/^[A-Z][A-Z\s&]{10,}$/.test(t) && paras.length < 3) {
        ...
        if (/^\d+\.\s+[A-Z]/.test(t) || /^SECTION\s+\d/i.test(t)) {
          paras.push(new Paragraph({
            heading: HeadingLevel.HEADING_1, spacing: { before: 300, after: 120 },
            children: [new TextRun({ text: t, bold: true, size: 26, font: 'Calibri', color: '1a2744' })],
          }))
          continue
        }
        if (/^\d+\.\d+/.test(t) || (/^[A-Z][A-Z\s&/]{5,}$/.test(t) && t.length < 60)) {
          paras.push(new Paragraph({
            heading: HeadingLevel.HEADING_2, spacing: { before: 200, after: 80 },
            children: [new TextRun({ text: t, bold: true, size: 22, font: 'Calibri', color: '2c3e50' })],
          }))
          continue
        }
        if (/^Version\s/i.test(t)) {
          paras.push(new Paragraph({
            ...
            children: [new TextRun({ text: t, italics: true, size: 20, font: 'Calibri', color: '6b7a99' })],
          }))
          continue
        }
        if (t.startsWith('- ') || t.startsWith('• ')) {
          paras.push(new Paragraph({
            ...
            children: [new TextRun({ text: t.slice(2), size: 21, font: 'Calibri' })],
          }))
          continue
        }
        if (t.startsWith('  - ') || t.startsWith('  • ')) {
          paras.push(new Paragraph({
            ...
            children: [new TextRun({ text: t.slice(4), size: 20, font: 'Calibri' })],
          }))
          continue
        }
        if (/^Q\d+[.:]/i.test(t)) {
    ...
    const doc = new Document({
      creator: orgName,
      title: title,
      description: 'Generated by OWASP GenAI Data Security Policy Generator',
      sections: [{
        properties: {
    ...
    saveAs(blob, `${orgName.replace(/\s+/g, '_')}_${title.replace(/\s+/g, '_')}.docx`)
  }

  const handleDownloadAll = async () => {
    for (const [id] of Object.entries(generatedPolicies)) {
      const pt = policyTypes.find(p => p.id === id)
      if (pt) await handleDownload(id, pt.title)
    }
  }
